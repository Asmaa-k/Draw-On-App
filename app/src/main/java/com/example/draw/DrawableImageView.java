...
public class DrawableImageView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    private List<Pen> mRedundantList = new ArrayList<Pen>();
    ...
    public void removeLastPath() {
        if (mPenList.size() > 0) {
            mRedundantList.add(mPenList.get(mPenList.size() - 1));
            mPenList.remove(mPenList.size() - 1);
            invalidate();
        }
    }

    public void readdLastPath() {
        if (mRedundantList.size() > 0) {
            mPenList.add(mRedundantList.get(mRedundantList.size() - 1));
            mRedundantList.remove(mRedundantList.size() - 1);
            invalidate();
        }
    }
